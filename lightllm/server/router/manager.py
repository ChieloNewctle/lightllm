...
class RouterManager:
    ...
    def __init__(self, weightdir, load_way, world_size, max_total_token_num, batch_max_tokens, running_max_req_size, eos_id,
        ...
        self.send_to_detokenization.connect(
            f"tcp://127.0.0.1:{detokenization_port}")
    ...
    def add_req(
        ...
        self.send_to_detokenization.send_pyobj(
            req.to_req_detokenization_state())
        return
    ...
    async def _init_batch(self, batch: Batch):
        ...
        rets = [
            self.model_rpcs[tp_rank].init_batch(
                batch.batch_id,
                reqs) for tp_rank in range(
                self.world_size)]
    ...
    async def _prefill_batch(self, batch):
        ...
        rets = [
            self.model_rpcs[tp_rank].prefill_batch(
                batch.batch_id) for tp_rank in range(
                self.world_size)]
    ...
    async def _decode_batch(self, batch: Batch):
        rets = [
            self.model_rpcs[tp_rank].decode_batch(
                batch.batch_id) for tp_rank in range(
                self.world_size)]
        ans = await asyncio.gather(*rets)
        if self.world_size != 1:
    ...
    async def _filter_batch(self, batch: Batch):
        ...
        rets = [
            self.model_rpcs[tp_rank].filter_batch(
                batch.batch_id,
                req_id_list) for tp_rank in range(
                self.world_size)]
        await asyncio.gather(*rets)
        return
    ...
    async def _merge_batch(self, batch1, batch2):
        rets = [
            self.model_rpcs[tp_rank].merge_batch(
                batch1.batch_id,
                batch2.batch_id) for tp_rank in range(
                self.world_size)]
        await asyncio.gather(*rets)
        return
    ...
    async def _remove_batch(self, batch):
        rets = [
            self.model_rpcs[tp_rank].remove_batch(
                batch.batch_id) for tp_rank in range(
                self.world_size)]
        await asyncio.gather(*rets)
        return
...
def start_router_process(
        args, router_port, detokenization_port, model_rpc_ports, mode, pipe_writer):
    try:
